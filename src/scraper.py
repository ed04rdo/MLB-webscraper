...
def main():
    ...
    global results_df
    if mode == 0:
        results_df = pd.concat([results_df,single_process()])
        write_file()
        exit()
...
def single_process():
    ...
    html = get(mlb_url+single_date) 
    soup = parse_html(html.text)
    tmp_df = format_data(soup, single_date)
...
def initialize_vars():
    ...
    mode = 1
    single_date = "2023-04-25"
    start_date = datetime.date(2021,2,1)
    end_date = datetime.date(2021,10,14)
    delta = end_date-start_date
    date_range = [start_date + datetime.timedelta(day) for day in range(0,delta.days+1)] if mode == 1 else None
